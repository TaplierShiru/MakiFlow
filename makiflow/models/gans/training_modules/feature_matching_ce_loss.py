...
class FeatureBinaryCETrainingModuleGenerator(GeneratorDiscriminatorBasic):
    ...
    def _build_feature_matching_loss(self):
        ...
        if self._use_feature_loss_only:
            ...
            ce_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=self._labels,
                                                              logits=self._logits,
                                                              name='generator_loss'
            )
            ...
            self._feature_matching_loss = tf.reduce_mean(ce_loss) + feature_loss * self._feature_scale
